...
extension BlastParser {
	...
	struct MergeQiime: ParsableCommand {
		static let configuration = CommandConfiguration(
			...
			usage: "blast_parser merge --qiime-import <qime-import> --blast-import <blast-import> [--output <output>] [--ncbi-tax-index <ncbi-tax-index>]",
			aliases: ["mrgq"] )
		
		@OptionGroup var options: Options
		
		@Option(name: [.short, .customLong("qiime-import")],
				help: "Path to the merged.tsv file generated by Qiime2.")
		var impQiime:String
		
		@Option(name: [.short, .customLong("blast-import")],
				help: "Path to the BLAST output file using a 13 columns format with following order: qsedid pident length evalue bitscore score nident saccver stitle qcovs staxids sscinames sskingdoms.")
		var impBlast:String
		...
		@Option(name: [.short, .customLong("ncbi-tax-index")],
				help: "Index to where the BLAST results are pasted, can be beginning or end [OPTIONAL, default = end]")
		var taxIndex:String?
		
		mutating func run() throws {
			guard let parser = BlastQiimeOutputParser(path: impBlast, asvs: impQiime, taxonomy: impQiime)
			else {
				throw RuntimeError("Could not find a valid Qiime 2 file to be merged with the BLAST hits file.")
			}
			//sets default name
			if outputFile == nil{
				var fileName = URL(fileURLWithPath: impQiime).deletingPathExtension().lastPathComponent
